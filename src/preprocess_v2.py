...
def pad_and_trim(text, window_size=16, start_token=START_TOKEN, stop_token=STOP_TOKEN, pad_token=PAD_TOKEN):
    words = text.split()
    num_words = len(words)
    trim_length = max((num_words - (window_size - 2)), 0)
    pad_length = max(0, ((window_size - 2) - num_words))
    padded_text = ' '.join([start_token] + words[:num_words - trim_length] + [stop_token] + [pad_token] * pad_length)
    return padded_text


def train_test_split(input_file='../data/nytfox_collate.json', test_split=0.05,
                     content_seq_leng=CONTENT_SEQ_LEN, title_seq_len=TITLE_SEQ_LEN):
    ...
    content_arr = [pad_and_trim(item['content'], content_seq_leng) for item in data]
...
def build_glove_embed_index(path_to_glove='../data/glove.6B/glove.6B.100d.txt',
    ...
    embeddings_index[stop_token] = np.random.normal(size=(embedding_size), scale=0.1)
